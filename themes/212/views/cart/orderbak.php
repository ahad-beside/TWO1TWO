  ...
  <div class="container">
    ...
    <div class="">
        ...
        <?php else: ?>
            <div class="col-md-12">
                <div class="order">
                    <div class="USCraft_title">Place Order</div>
                    <ul class="accordion-step checkoutstep f1-steps">
                        ...
                        <li data-step="step-1" class="f1-step item login-item <?= (Yii::app()->user->isGuest) ? 'enable' : 'disable' ?>">
                            <div class="heading f1-step-icon" style="padding-top:9px;" data-pos-val='25'>
                        ...
                        <li data-step="step-3" class="f1-step item shipping_method <?= (Yii::app()->user->isGuest) ? 'disable' : 'enable' ?>">
                            <div class="heading f1-step-icon" data-btn-click="chkBillingShipping" style="padding-top:9px; padding-right:2px;">
                            ...
                            <div class="content">
                                <div class="row">
                                    <div class="col-md-6 wristrdo newcustomer">
                                        ...
                                        <div class="radio new_radio" style="padding-bottom:10px;">
                                            
                                            <input type="radio" id="register-acc" class="new-customer css-checkbox" name="new_customer" value="Register">
                                            <label for="register-acc" class="css-label radGroup1 radio-inline radio">Register Account</label>
                                        </div>

                                        <div class="radio new_radio">
                                        ...
                                        <div style="display:none">
                                           <?php /* if (count($data['shippingMethod']) > 0): ?>
                                            ...
                                            foreach ($data['shippingMethod'] as $sm): $x++;
                                                ?>
                                                <div class="radio wristrdo new_radio">
                                                    <label for="radio-dm-<?= $x ?>" class="css-label radGroup1 radio-inline radio">
                                                    <input value="<?= $sm->id ?>" type="radio" name="shipping_method" id="radio-dm-<?= $x ?>" class="css-checkbox" <?php
                                                    if (count($data['shippingMethod']) == 1) {
                                                        echo 'checked="checked"';
                                                    }
                                                    ?>>
                                                     <span><?= $sm->name ?> (<?= ShippingSettings::model()->calShippingPrice($sm, $session['cart']) ?>)</span></label>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php else: ?>
...
<?php endif; ?>
        ...
        $('input[name="register_email_address"]').donetyping(function () {
            var mail = $(this);
            if (mail.val().trim() != '') {
        ...
        $('input[name="repassword"]').donetyping(function () {
...
<?php } ?>
        ...
        $('.chkBillingShipping').click(function (e) {
            ...
            var s_productWeight = $('input[name="productWeight"]').val().trim();
            var s_productTypeShipping = $('input[name="productTypeShipping"]').val().trim();
            ...
            $.post('<?= $this->createUrl('//cart/chkBillingInput'); ?>', {name: b_name, street_address: b_street_address, city: b_city,state: b_state, country: b_country, pincode: b_pincode, phone: b_phone}, function (data) {
                ...
                $.post('<?= $this->createUrl('//cart/chkShippingInput'); ?>', {name: s_name, street_address: s_street_address, city: s_city,state: s_state, country: s_country, pincode: s_pincode, phone: s_phone}, function (data) {
                    ...
                    if(s_productTypeShipping=='Regular'){
                    chkShippingMethodPrice(s_productWeight,s_state);
                }else{
                    //alert('Custom');
                    $('.shippingMethodShow').html('<div class="col-md-4"><div class="radio wristrdo new_radio shippingTotalCss"><label for="radio-dm-1" class="css-label radGroup1 radio-inline radio"><input value="0.00" name="shipping_method" id="radio-dm-1" class="css-checkbox" type="radio" checked="checked"><span> Shipping Charge Already Added</span></label></div></div>');
...
<?php endif; ?>
        ...
        function chkShippingMethodPrice(s_productWeight,s_state) {
        //alert(s_productWeight);
        $.post('<?= $this->createUrl('//cart/calculateShippingValue'); ?>', {weight: s_productWeight, state: s_state}, function (data) {
            $('.shippingMethodShow').html(data);
        });
        }
...
<script type="text/javascript">
    ...
    function updateCheckOutPrice() {
        ...
        $.post('<?= $this->createUrl('loadTotalRows') ?>', {}, function (data) {
            $('.checkout-table').find('tfoot').html(data);
            //alert(data);
        });
