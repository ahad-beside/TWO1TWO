    ...
    <div class="row">
      <div class="white-bg">
        ...
        <form action="#" method="post" id="addtocart">
          <input type="hidden" name="item_from" value="Product"/>
          ...
          <input type="hidden" class="sebscriptionMonth" data-rule="subscriptionMonth" value="1" name="subscriptionMonth">
           <input class="productOriginalPrice" type="hidden" name="product-price" value="<?php echo $price ?>"/>
        <div class="col-md-8 col-sm-6 col-xs-12">
          <div class="info-panel">
            ...
            <?php if(count($data['productOption'])>0){?>
            <h5 class="sub-title" style="margin-top: 12px;">Choose Option</h5>
          <div class="">            
                <?php foreach($data['productOption'] as $rowProOption):?>
                 
                  <div class="col-md-6" style="margin-bottom: 12px;">
                    <div class="row" style="border: 1px solid #ccc; padding: 5px; margin-right: 0.5px;">
                    <div class="col-md-2 col-sm-4">
                      <div class="checkbox checkbox-success" style="padding-top: 20px;">
                        <input id="checkbox<?= $rowProOption->id?>" class="select-option styled" type="checkbox" name="productOption[]" value="<?= $rowProOption->id?>" data-price="<?= $rowProOption->price?>">
                        <label for="checkbox<?= $rowProOption->id?>">                            
                        </label>
                    </div>
                    </div>
                    <div class="col-md-4 col-sm-4"><img src="<?=Yii::app()->easycode->showOriginalImage($rowProOption->image,'/product/');?>" width="80" height="80" alt=""></div>
                    <div class="col-md-6 col-sm-4">
                      <div><b><?= $rowProOption->name?></b></div>
                      <div><span style="font-style: italic;">$ <?= $rowProOption->price?></span></div>                      
                    </div>
                  </div>
                  </div>
              <?php endforeach;?>
    ...
    <div class="row">
      <div class="col-md-12 page-service-details">
        <div class="service-what-our-passengers-say section">
          ...
          <div id="writeReviewFrom" class="col-md-6" style="background-color:#eee;font-size:14px;display: none; margin-bottom:30px;">
          <h3 style="margin-bottom:0px; padding:6px 0px;">Write your review</h3>
       ...
       <div class="leave-review">
        ...
        echo $form->hiddenField($modelReview, 'review_type', array('size' => 50, 'maxlength' => 50, 'class' => 'form-control','value' => 'Product'));
...
<script>
  ...
  $(document).on('click', '.selectAllCheckBox', function () {
        var obj = $(this);
        var relClass = obj.attr('data-rel-class');
        $('.' + relClass).prop('checked', obj.prop("checked"));
        getSelectedOption();
    });

    $(document).on('click', '.select-option', function (){
      var originalPrice=$('.productOriginalPrice').val();
      optionPrice=0;
      if ($(this).is(':checked')){
        optionPrice +=parseFloat($(this).attr('data-price'));
      }
      else{
        optionPrice = optionPrice - parseFloat($(this).attr('data-price'));
      }
      var totalPriceWithOption=parseFloat(optionPrice) + parseFloat(originalPrice);
      $('.productOriginalPrice').val(totalPriceWithOption.toFixed(2));
      $('.price').html('$'+totalPriceWithOption.toFixed(2));
    });
