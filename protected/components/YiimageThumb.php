...
class YiimageThumb{
    ...
    public function render($image, $options = array(),$returnImg=true)
    {
        ...
        return $this->createThumb($returnImg);
    }
    ...
    private function createThumb($returnImg=true)
    {
        ...
        if(file_exists($this->thumb['file'])){
            $img = CHtml::image(Yii::app()->request->baseUrl.$file_url, $this->imgAlt, $this->imgOptions);
            $imgPath = Yii::app()->request->baseUrl.$file_url;
