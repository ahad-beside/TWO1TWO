...
class Uploader {
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        ...
        /*change filename added by rajib*/
        $pathinfo = pathinfo($this->fileName);
        $this->fileName = Yii::app()->session['customId'].'_'.time().'_'.rand(11,999).'.'.strtolower($pathinfo['extension']);
        //$this->fileName = $pathinfo['filename'].'_'.Yii::app()->session['customId'].'_'.time().'.'.strtolower($pathinfo['extension']);
        /*change filename added by rajib*/
        ...
        return array('result'=>true,'filename'=>$this->fileName);
    }
