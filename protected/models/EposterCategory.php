 ...
 */
class EposterCategory extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			'eposterLists' => array(self::HAS_MANY, 'EposterList', 'category_id'),
		);
	}
	...
	public static function makeLink($id){
        ...
        if(count($info)>0){
            ...
            return Yii::app()->createAbsoluteUrl('//eposterCategory/'.$info->slug);
        }else{
            return '#';
        }
    }
    public static function makeLinkNew($id){
        ...
        if(count($info)>0){
            //return Yii::app()->createUrl('//category/'.$id,array('name'=>$info->name));
            //return Yii::app()->createAbsoluteUrl('//category/'.$info->slug);
            return Yii::app()->createUrl('//eposterCategory/view/'.$info->slug);
        }else{
            return '#';
        }
    }
