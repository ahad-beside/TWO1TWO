...
class Products extends CActiveRecord {
    ...
    public function rules(){
        ...
        return array(
            array('name, metatag_title, model, sku, price,reseller_price, quantity, update_by, update_time, slug,quick_view', 'required'),
            ...
            array('description,specification,feature,benifits, metatag_description, metatag_keywords, image, seo_keyword, sort_order, featured,download', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, name, description, metatag_title, metatag_description, metatag_keywords, image, model, sku, price, quantity, seo_keyword,status, sort_order, update_by, update_time,quick_view', 'safe', 'on' => 'search'),
        );
    }
    ...
    public static function makeLink($id){
        return Yii::app()->createAbsoluteUrl('//products/' . self::model()->findByPk($id)->slug);
    }

    public static function getProductPrice($pid, $option = array(), $qty='') {
        $optionPrice=0;
        if (count($option) > 0) {
            for($i=0;$i<count($option);$i++):
                $chkOpPrice = ProductOption::model()->find('id=:id', array(':id' => $option[$i]));
                if (count($chkOpPrice) > 0){
                        $optionPrice += $chkOpPrice->price;
                    }
            endfor;
        }
    ...
    public static function getFeaturedProducts($catId = ''){
    ...
    public static function getRelatedProducts($makeOr,$id) {
        $command = Yii::app()->db->createCommand();
        $command->select("p.id, p.name, p.image, p.price, p.update_time, pc.category_id");
    ...
    public static function getCategoryFilter($cat = '') {
        $categoryList = array();
        if ($cat == '') {
            $allcat = Category::model()->findAll('status=:status and parent is :parent order by sort_order', array(':status' => 1, ':parent' => Null));
           foreach($allcat as $v):
           ...
           endforeach;
            ...
            $childCat = Category::model()->findAll('status=:status and parent=:parent order by sort_order', array(':status' => 1, ':parent' => $catInfo->id));
    ...
    public static function getProductChild($id) {
            $childCat = Category::model()->findAll('status=:status and parent=:parent order by sort_order', array(':status' => 1, ':parent' => $id));
            return $childCat;
    }
    ...
    public static function deleteProductImages($id) {
        ...
        if ($model->image != '')
            Yii::app()->easycode->deleteFile($model->image,'product');
        ...
        if ($id != '' && $id > 0) {
            ...
            if (count($info) > 0) {
                foreach ($info as $row):
                    Yii::app()->easycode->deleteFile($row->image,'product');
                endforeach;
            }
        }
    }
    ...
    public static function getUMayAlsoLike_old($id) {
        ...
        if (count($getCategory) > 0) {
            foreach ($getCategory as $cat):
                ...
                $categoryId [] = "pc.category_id='" . $cat->category_id . "'";
            endforeach;
        }
        ...
        $command->where(array('and', 'p.outofstock_status="In Stock"', $categoryId));
        $command->andwhere('p.status=:status', array(':status' => 1));
        $command->andwhere('p.id!=:pid', array(':pid' => $id));
    ...
    public static function chkAttributeInProduct($productId, $nameOfAttribute){
        $attribute = Attribute::model()->find('name=:name',array(':name'=>$nameOfAttribute));
        if(count($attribute)>0):
            return ProductsAttribute::model()->find('product_id=:pid and attribute_id=:aid', array(':pid'=>$productId, ':aid'=> $attribute->id))->message;
        endif;
    }
