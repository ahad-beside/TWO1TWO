...
class ServiceController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('index', 'view', 'umayalsolike','viewTest','getSubscriptionPrice'),
                'users' => array('*'),
            ),
            array('allow',
    ...
    public function actionIndex(){
        $this->redirect(array('//serviceCategory/all'));
    }
    public function actionGetSubscriptionPrice(){
        $subscription=$_POST['subscription'];
        $servicePrice=$_POST['servicePrice'];
        if($subscription!=''){
        $subscriptionMonth=Subscription::model()->findByPk($subscription)->total_month;
        echo CJSON::encode(array('price'=>(number_format($servicePrice*$subscriptionMonth,2)),'totalMonth'=>$subscriptionMonth));
    }else{
        //echo (number_format($servicePrice,2));
        echo CJSON::encode(array('price'=>(number_format($servicePrice,2)),'totalMonth'=>1));
    }
    }

    public function actionView($name) {
        $model = Service::model()->find('slug=:slug',array(':slug'=>$name));
        $this->pageTitle = $model->name.' - '.Yii::app()->name;
        $data['subscription']=Subscription::model()->findAll();
    ...
    //echo $data['totalRating']->rating_point;exit;
        $data['images'] = ServiceImage::model()->findAll('product_id=:id', array(':id' => (int) $id));
        $data['download'] = ServiceDownload::model()->findAll('product_id=:id', array(':id' => (int) $id));
        $data['categoryId']=CategoryService::model()->findAll("product_id=".(int) $id);
        $i=0;
        $data['makeOr']='';
        ...
        endforeach;
        $data['relatedProduct']=Service::model()->getRelatedService($data['makeOr'],(int) $id);
